...
<form name="registerForm" id="registerForm" onsubmit="return registerUser(this);">
			<div class="e-tabbox main-expand-form">
		        ...
		        <div class="formBoxItem">
	              ...
	              <div class="formBoxItemR">
	               <input class="easyui-textbox" id="name" name="organame" style="width:200px;height: 25px;" data-options="required:true,validType:'unique[\'${cxt}/registerOrganization/ajax/checkUpdateName.action?id=${register.organizationId}\',\'name\',\'机构名称已存在\']'" type="text"/>
             	  </div>
	            </div>
	            <div id="create" >
		        ...
		        <div class="formBoxItem">
	              ...
	              <div class="formBoxItemR">
	              <input class="easyui-textbox" id="name" name="loginName" style="width:200px;height: 25px;" data-options="required:true,validType:'unique[\'${cxt}/user/ajax/findLoginName.action?id=${register.userId}\',\'loginName\',\'管理员名称已存在\']'" type="text"/>
	              </div>
	            </div>
	            <div class="formBoxItem">
	 	...
	 	<div class="e-tab-btnbox">
             ...
             <input name="organizationId" value="${register.organizationId}" type="hidden"/>
        </div>
	</form>
<script  type="text/javascript">  
window.onload=function(){
	$(".e-tabbox").height($(this).height()-$(".e-tab-btnbox").height()-100);
	$("#registerForm").form("load",{
		organame:"${register.organame}",
...
function submit(){
	...
	function registerUser(form1){
		...
		try{
			...
			if(c){
				jQuery("#parameterMsg").html("请选择功能点");
			}else{
			...
			if(form1.form('validate') && !c){ 
				...
				jQuery.post("${cxt}/registerOrganization/ajax/update.action",params,function call(result){
					if(result.status){
						top.showMsg("success","机构修改成功");
						parent.callbackPage();
						cancelWindow();
					}else{
