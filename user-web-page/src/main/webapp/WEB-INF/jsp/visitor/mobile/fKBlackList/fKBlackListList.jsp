...
<script type="text/javascript">
	$(function(){
		$("#datagridList").datagrid({
			nowrap: true,
			url:"${cxt}/vistor/fKBlackList/ajax/list.action",
			collapsible:true,
			showfolder:true,
			border:false,
			fitColumns:true,
	...
	function addFKBlackList(){
		$("#win").window({
			...
			width:800,
			height:600,
			iframe:"${cxt}/vistor/fKBlackList/preAdd.action",
			modal:true
		});
	}
	...
	function editFKBlackList(id){
		$("#win").window({
			...
			width:800,
			height:600,
			iframe:"${cxt}/vistor/fKBlackList/preUpdate.action?id="+id,
			modal:true
		});
	}
	...
	function deleteFKBlackLists(){
        ...
        if(confirm("确认是否删除?")){
			jQuery.post("${cxt}/vistor/fKBlackList/ajax/delete.action",param, function call(data){
	...
	function deleteFKBlackList(id){
	     if(confirm("确认是否删除?")){
			...
			jQuery.post("${cxt}/vistor/fKBlackList/ajax/delete.action",{"ids":id}, function call(data){
	...
	function importBlackList(){
		if(confirm("确认是否导入到!!")){
			 $("#win").window({
			    ...
			    width:800,
			    height:500,
			    iframe:"${cxt}/vistor/fKBlackList/preImportBlackList.action",
			    modal:true
			}); 
		}
	}
