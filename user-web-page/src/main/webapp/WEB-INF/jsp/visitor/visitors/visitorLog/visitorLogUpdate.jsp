...
<head>
  <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<%@include file="../common/common.jsp"%>
 	<title>修改访客记录--${system_name}</title>  
</head>
<body>
<div>
<!--内容开始-->
	<form action="#" method="get" name="visitorLogForm" id="visitorLogForm" onsubmit="return updateVisitorLog(this)">
	<div id="easyui-tabs">
		<!--tab开始-->
		<div title="第一teb" class="main-tabbox" style="height:310px;">
			<div class="main-formbox">
				...
				<div class="e-tabbox">
					...
					<tr><th class="e-form-th" style="width:92px;">离开时间</th>
						<td class="e-form-td">
		...
		<input type="hidden" id="id" name="id" value="${visitorLog.id}"/>
	...
	window.onload=function(){
		//加载数据
 		$("#visitorLogForm").form("load",{
			...
			startappStatus:"${visitorLog.startappStatus}",
			agenciesId:"${visitorLog.agenciesId}",
	...
	function updateVisitorLog(){
		try{
			if($("#visitorLogForm").form("validate")){
				WaitingBar.getWaitingbar("updatevisitorLog","数据修改中，请等待...").show();
				var params=jQuery("#visitorLogForm").serialize();
				jQuery.post("${cxt}/visitorLog/ajax/update.action",params,function call(data){
					WaitingBar.getWaitingbar("updatevisitorLog").hide();
					if(data.status){
						showMsg("info","编辑访客记录成功");
						top.callbackPage();
						cancelWindow();
					}else{
						if(data.message){
