...
<script type="text/javascript">
	$(function(){
		$("#datagridList").datagrid({
			nowrap: true,
			url:"${cxt}/mobile/punchRecord/ajax/list.action",
			collapsible:true,
			showfolder:true,
			border:false,
			fitColumns:true,
			...
			columns:[[
				{field:"ck",checkbox:true},
						...
						{title:"照片",field:"dataUrl",width:150,formatter:function(value,rec){
							var buttonHtml=[];
							if(rec.dataUrl){
								buttonHtml.push("<img onclick=\"bigImg('"+rec.data+"');\" width=\"50px\" height=\"50px\" src=\""+rec.dataUrl+"\">"," ");
							}
							return "<span style=\"color:red\">"+buttonHtml.join("")+"</span>";
						}},
						{title:"抓拍照片",field:"panoramaImgUrl",width:150,formatter:function(value,rec){
							var buttonHtml=[];
							if(rec.dataUrl){
								buttonHtml.push("<img onclick=\"bigImg('"+rec.panoramaImg+"');\" width=\"50px\" height=\"50px\" src=\""+rec.panoramaImgUrl+"\">"," ");
							}
							return "<span style=\"color:red\">"+buttonHtml.join("")+"</span>";
						}},
						{title:"比分",field:"score",width:100},
						...
						buttonHtml.push("<a href=\"javascript:preAddWhite('"+rec.id+"');\">添加至白名单</a>"," ");
						return "<span style=\"color:red\">"+buttonHtml.join("")+"</span>";
					}
				}
			]],
	...
	function bigImg(data){
		$("#win").window({
			...
			width:400,
			height:450,
			iframe:"${cxt}/mobile/punchRecord/preBigImg.action?data="+data,
			modal:true
		});
	}
	function preAddWhite(id){
		$("#win").window({
			title:"添加至白名单",
			width:800,
			height:600,
			iframe:"${cxt}/mobile/punchRecord/preAddWhite.action?id="+id,
			modal:true
		});
	}
