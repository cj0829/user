...
<div>
<!--内容开始-->
	<form action="#" method="get" name="dactyForm" id="dactyForm" onsubmit="return updateDacty(this)">
	<div id="easyui-tabs">
		<!--tab开始-->
		...
		<input type="hidden" id="id" name="id" value="${dacty.id}"/>
...
<script type="text/javascript">
	window.onload=function(){
		//加载数据
 		$("#dactyForm").form("load",{
			ac_attenddacty:"${dacty.ac_attenddacty}",
			id:"${dacty.id}",
			userId:"${dacty.userId}",
			dacty:"${dacty.dacty}",
		});
	...
	//提交表单的方法
	function updateDacty(){
		try{
			if($("#dactyForm").form("validate")){
				WaitingBar.getWaitingbar("updatedacty","数据修改中，请等待...","${jsPath}").show();
				var params=jQuery("#dactyForm").serialize();
				jQuery.post("${cxt}/mobile/dacty/ajax/update.action",params,function call(data){
					WaitingBar.getWaitingbar("updatedacty").hide();
					if(data.status){
