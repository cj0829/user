...
<script type="text/javascript">
 	...
 	WebUploaderError.put("F_EXCEED_SIZE","请上传小于2M的图片！");
 	WebUploaderError.put("Q_EXCEED_SIZE_LIMIT","请上传小于2M的图片！");
 	...
 	var uploader1 = WebUploader.create({
 		...
 		server : "${cxt}/sev/amFace/ajax/collectFace.action?userId=${userId}&index=1",
 		fileVal : "image",
 		duplicate : true,
 		fileNumLimit : 1,
 	...
 	var uploader2 = WebUploader.create({
 		...
 		swf : "${cxt}/js/webuploader/Uploader.swf",
 		server : "${cxt}/sev/amFace/ajax/collectFace.action?userId=${userId}&index=2",
 		fileVal : "image",
 		duplicate : true,
 		fileNumLimit : 1,
 		fileSingleSizeLimit : 2*1024*1024, 
	...
	uploader2.on('uploadBeforeSend', function(file, data, header) {
		data["random"]=Math.random();
	});
	...
	//-->上传 第三图片
 	var uploader3 = WebUploader.create({
 		...
 		swf : "${cxt}/js/webuploader/Uploader.swf",
 		server : "${cxt}/sev/amFace/ajax/collectFace.action?userId=${userId}&index=3",
 		fileVal : "image",
 		duplicate : true,
 		fileNumLimit : 1,
 		fileSingleSizeLimit : 2*1024*1024, 
 	...
 	uploader3.on('uploadBeforeSend', function(file, data, header) {
		data["random"]=Math.random();
	});
