		...
		<div class="e-tabchange-bd">
			...
			<table width="100%">
				<tr>
					...
					<ul>
											...
											<li class="e-search-td"><label class="mr8 lab" style="width:90px;">组id:</label><input id="groupId" class="main-inp" type="text" type="text"style="width:115px; height:25px;"/></li>
											<li class="e-search-td"><label class="mr8 lab" style="width:90px;">是否必须打卡:</label><input id="isMust" class="main-inp" type="text" type="text"style="width:115px; height:25px;"/></li>
										</ul>
					</td>
			...
			<div class="main-tableContainer cl-margin">
				<div class="e-tabbtn-box mb2">
					<button class="tabbtn fr" type="button" onclick="addUserGroup();">新建</button>
					<button class="tabbtn fr mr5" type="button" onclick="deleteUserGroups();">删除</button>
				</div>
...
<script type="text/javascript">
	$(function(){
		$("#datagridList").datagrid({
			nowrap: true,
			url:"${cxt}/mobile/userGroup/ajax/list.action",
			collapsible:true,
			showfolder:true,
			border:false,
			fitColumns:true,
			...
			columns:[[
				{field:"ck",checkbox:true},
						...
						{title:"用户组关系",field:"ac_usergroup",width:150},
						{title:" id",field:"id",width:150},
						{title:"用户id",field:"userId",width:150},
						{title:"组id",field:"groupId",width:150},
						{title:"是否必须打卡",field:"isMust",width:150},
						{title:"操作",field:"id_1",width:200,selected:true,formatter:function(value,rec){
						var buttonHtml=[];
						buttonHtml.push("<a href=\"javascript:editUserGroup('"+rec.id+"');\">编辑</a>"," ");
						buttonHtml.push("<a href=\"javascript:deleteUserGroup('"+rec.id+"');\">删除</a>"," ");
	...
	function queryParams(){
		try{
			...
			"groupId":"like:$"+$("#groupId").val(),
			"isMust":"like:$"+$("#isMust").val(),
};
		}catch (e) {}
		return {};
	};
	...
	function addUserGroup(){
		$("#win").window({
			...
			width:800,
			height:600,
			iframe:"${cxt}/mobile/userGroup/preAdd.action",
			modal:true
		});
	}
	...
	function editUserGroup(id){
		$("#win").window({
			...
			width:800,
			height:600,
			iframe:"${cxt}/mobile/userGroup/preUpdate.action?id="+id,
			modal:true
		});
	}
	...
	function deleteUserGroups(){
        ...
        if(confirm("确认是否删除?")){
			jQuery.post("${cxt}/mobile/userGroup/ajax/delete.action",param, function call(data){
	...
	function deleteUserGroup(id){
	     if(confirm("确认是否删除?")){
			...
			jQuery.post("${cxt}/mobile/userGroup/ajax/delete.action",{"ids":id}, function call(data){
