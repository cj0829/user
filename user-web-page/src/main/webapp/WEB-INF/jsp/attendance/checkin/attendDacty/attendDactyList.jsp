		...
		<div class="e-tabchange-bd">
			...
			<div class="main-tableContainer cl-margin">
				<div class="e-tabbtn-box mb2">
					<button class="tabbtn fr" type="button" onclick="addAttendDacty();">新建</button>
					<button class="tabbtn fr mr5" type="button" onclick="deleteAttendDactys();">删除</button>
				</div>
...
<script type="text/javascript">
	$(function(){
		$("#datagridList").datagrid({
			nowrap: true,
			url:"${cxt}/attendance/attendDacty/ajax/list.action",
			collapsible:true,
			showfolder:true,
			border:false,
			fitColumns:true,
			...
			columns:[[
				{field:"ck",checkbox:true},
						...
						var buttonHtml=[];
						buttonHtml.push("<a href=\"javascript:editAttendDacty('"+rec.id+"');\">编辑</a>"," ");
						buttonHtml.push("<a href=\"javascript:deleteAttendDacty('"+rec.id+"');\">删除</a>"," ");
	...
	function addAttendDacty(){
		$("#win").window({
			title:"新建",
			width:800,
			height:600,
			iframe:"${cxt}/attendance/attendDacty/preAdd.action",
			modal:true
		});
	}
	
	// 编辑
	function editAttendDacty(id){
		$("#win").window({
			title:"编辑",   
			width:800,
			height:600,
			iframe:"${cxt}/attendance/attendDacty/preUpdate.action?id="+id,
			modal:true
		});
	}
	
	//删除全部
	function deleteAttendDactys(){
        ...
        if(confirm("确认是否删除?")){
			jQuery.post("${cxt}/attendance/attendDacty/ajax/delete.action",param, function call(data){
	...
	// 删除
	function deleteAttendDacty(id){
	     if(confirm("确认是否删除?")){
			WaitingBar.getWaitingbar("delete","删除数据中，请等待...","${jsPath}").show();
			jQuery.post("${cxt}/attendance/attendDacty/ajax/delete.action",{"ids":id}, function call(data){
