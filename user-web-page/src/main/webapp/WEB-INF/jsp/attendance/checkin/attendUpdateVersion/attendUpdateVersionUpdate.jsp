...
<div>
<!--内容开始-->
	<form action="#" method="get" name="attendUpdateVersionForm" id="attendUpdateVersionForm" onsubmit="return updateAttendUpdateVersion(this)">
	<div id="easyui-tabs">
		<!--tab开始-->
		...
		<input type="hidden" id="id" name="id" value="${attendUpdateVersion.id}"/>
...
<script type="text/javascript">
	window.onload=function(){
		//加载数据
 		$("#attendUpdateVersionForm").form("load",{
			ac_updateversion:"${attendUpdateVersion.ac_updateversion}",
			id:"${attendUpdateVersion.id}",
			url:"${attendUpdateVersion.url}",
			newVersion:"${attendUpdateVersion.newVersion}",
			content:"${attendUpdateVersion.content}",
			agenciesId:"${attendUpdateVersion.agenciesId}",
		});
	};
	...
	//提交表单的方法
	function updateAttendUpdateVersion(){
		try{
			if($("#attendUpdateVersionForm").form("validate")){
				WaitingBar.getWaitingbar("updateattendUpdateVersion","数据修改中，请等待...","${jsPath}").show();
				var params=jQuery("#attendUpdateVersionForm").serialize();
				jQuery.post("${cxt}/attendance/attendUpdateVersion/ajax/update.action",params,function call(data){
					WaitingBar.getWaitingbar("updateattendUpdateVersion").hide();
					if(data.status){
