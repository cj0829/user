				...
				switch (typeof(value)) {
					...
					case "object":
						if (jwplayer.utils.exists(value)) {
							value = jwplayer.utils.jsonToString(value);
						}
						break;
				}
				if (isArray) {
...
(function(jwplayer) {
	jwplayer.events = {
		...
		JWPLAYER_MEDIA_INTERVAL:'jwplayerMediaInterval',
		JWPLAYER_MEDIA_MESSAGEINTERVAL:'jwplayerMediaMessageInterval',
	...
	var _flash = jwplayer.embed.flash = function(_container, _player, _options, _loader, _api) {
		...
		function jsonToFlashvars(json) {
			...
			for (var key in json) {
				if (typeof(json[key]) == "object") {
					flashvars += key + '=' + encodeURIComponent("[[JSON]]"+jwplayer.utils.jsonToString(json[key])) + '&';
				} else {
		...
		this.embed = function() {		
			...
			if (utils.isIE()) {
				...
				html += '<param name="flashvars" value="' + jsonToFlashvars(params) + '">';
				html += '</object>';
...
(function(jwplayer, undefined) {
	...
	var api = jwplayer.api = function(container) {
		...
		var _eventMapping = {
			...
			onIntervalTime:events.JWPLAYER_MEDIA_INTERVAL,
			onMessageInterval:events.JWPLAYER_MEDIA_MESSAGEINTERVAL,
		...
		function _stateCallback(state) {
			return function(args) {
				var newstate = args.newstate, oldstate = args.oldstate, position = args.position
				if (newstate == state) {
...
(function(jwplayer) {
	...
	jwplayer.api.instream = function(_api, _player) {
		...
		this.onIntervalTime = function(callback) {
			return this.eventListener(events.JWPLAYER_MEDIA_INTERVAL, callback);
		};
		this.onMessageInterval = function(callback) {
			return this.eventListener(events.JWPLAYER_MEDIA_MESSAGEINTERVAL, callback);
		};
