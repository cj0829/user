...
var Photo = {
    ...
    getStyle : function( elem, name ) {
		 if (elem.style[name])
			 ...
			 name = name.replace(/([A-Z])/g,"-Photo1");
...
Util.Drag.prototype = {
	...
	init : function(container,drag,options){
		...
		this.Move = Photo.bindAsEventListener(this,this.move);
		this.Stop = Photo.bindAsEventListener(this,this.stop);
		this.drag.style.cursor = "move";	
		Photo.addEvent(this.drag,Photo.bindAsEventListener(this,this.start),"mousedown");
	},
	start : function(E){	
		this.x = E.clientX - this.drag.offsetLeft;
		this.y = E.clientY - this.drag.offsetTop;		
		Photo.addEvent(document,this.Move,"mousemove");
		Photo.addEvent(document,this.Stop,"mouseup");
		this.startFunc.call(this,E, this.container);
		if(document.all){
			Photo.addEvent(this.drag, this.Stop, "losecapture");
			this.drag.setCapture();
			E.cancelBubble = true;
		}else{
			Photo.addEvent(window, this.Stop, "blur");
			E.preventDefault();
		}
	},
	move : function(E){
	...
	stop : function(E){
		Photo.removeEvent(document,this.Move,"mousemove");
		Photo.removeEvent(document,this.Stop,"mouseup");
		this.endFunc.call(this,E, this.container);
		if(document.all){
			Photo.removeEvent(this.drag, this.Stop, "losecapture");
			this.drag.releaseCapture();
			E.cancelBubble = true;
		}else{
			Photo.removeEvent(window, this.Stop, "blur");
		}
	}
};
...
Slider.prototype = {
	...
	init : function(options){
		...
		this.icontainer = Photo.get(options.icontainer);
		this.idrag = Photo.get(options.idrag);
		if(options.plusBtn){
			this.plusBtn = Photo.get(options.plusBtn);
			this.reduceBtn = Photo.get(options.reduceBtn);			
		}	
		this.direction = options.direction;
		if(options.content){
			this.content = Photo.get(options.content);
			this.panel = Photo.get(options.panel);
	...
	addEvent : function(){
		...
		self.fixedBug();
		if(self.plusBtn){
			Photo.addEvent(self.plusBtn, function(){
				self.goToward("up", self);
			}, "mousedown");
			Photo.addEvent(self.plusBtn, function(){
				clearTimeout(self.timer);
			}, "mouseup");	
			Photo.addEvent(self.reduceBtn, function(){
		...
		Photo.addEvent(self.icontainer, function(event){
		...
		new Util.Drag(self.icontainer, self.idrag, {
			...
			moveFunc : function(E, idrag, icontainer){
				if(self.content){
					self.callback(parseInt(Photo.getStyle(idrag, self.direction)), self.Max[self.direction]);
				}
			},
			endFunc : function(E){		
				setTimeout(function(){
	...
	run : function (self){
		...
		var posUp = Math.min(Math.max(parseInt(Photo.getStyle(self.idrag, self.direction)) + acc,self.Min[self.direction]),self.Max[self.direction]);
		if(self.content){
	...
	mouseClick : function(event, self){
		var pos = Photo.getPos(event),
			offset = Photo.getS(self.idrag),
			dis = pos[self.direction == "top" ? "pageY" : "pageX"] - offset[self.direction],
			top = parseInt(Photo.getStyle(self.idrag, self.direction)),
		...
		if(self.content){
			self.callback(parseInt(Photo.getStyle(self.idrag, self.direction)), self.Max[self.direction]);	
		}
		Photo.preventDefault(event);	
	},	
	scrollFunc : function scrollFunc(event, self){
		self.dec = Photo.getDetail(event);
		self.isScroll = true;
		self.run();
		Photo.preventDefault(event);
	},	
	keydown : function scrollFunc1(event, self){
		var which = Photo.getWhich(event);
		if(which === 38){
			self.dec = - 10;
		}
	...
	fixedBug : function(){
		...
		if(!document.all){
			Photo.addEvent(this.icontainer, function(){
