...
function UploadVideo(target,serverUrl) {
		...
		function addFile(file) {
         ...
         if (file.getStatus() === 'invalid') {
             ...
             if ('|png|jpg|jpeg|bmp|gif|'.indexOf('|'+file.ext.toLowerCase()+'|') == -1) {
                 $imgWrap.empty().addClass('notimage').append('<i class="file-preview file-type-' + file.ext.toLowerCase() + '"></i>' +
...
//				$placeHolder.addClass('element-invisible');
		...
		uploader.on('uploadSuccess', function(file, ret) {
			...
			try {
				...
				if (json.status == 1) {
					_this.uploadVideoList.push(json);
	...
	getInsertList : function() {
		...
		var width = $('#upload_width').val() || 420;
        var height = $('#upload_height').val() || 280;
		for (i = 0; i < this.uploadVideoList.length; i++) {
			data = this.uploadVideoList[i];
			list.push({
				url : this.serverUrl+"/attachment/flv/"+data.id+"/video.flv",
				width:width,
                height:height,
				alt : data.fileName,
//				floatStyle : align
