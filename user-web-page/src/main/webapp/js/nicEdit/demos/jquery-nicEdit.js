...
(function($) {
	...
	var bkLib = {
		isMSIE : _ua.indexOf('msie') > -1 && _ua.indexOf('opera') == -1,
		...
		onDomLoadedgoods : function(fireThis) {
			this.domLoad.push(fireThis);
			if (document.addEventListener) {
				document.addEventListener("DOMContentLoaded", bkLib.domLoaded, null);
			} else if(bkLib.isMSIE) {
	...
	var nicEditorConfig = bkClass.extend({
		...
		iconList : {"fontFamily":29,"fontSize":28,"xhtml":1,"bgcolor":2,"forecolor":3,"bold":4,"center":5,"hr":6,"indent":7,"italic":8,"justify":9,"left":10,"ol":11,"outdent":12,"removeformat":13,"right":14,"save":25,"strikethrough":16,"subscript":17,"superscript":18,"ul":19,"underline":20,"image":21,"link":22,"unlink":23,"close":24,"arrow":26,"upload":27}
...
//			this.ne.fireEvent("buttonOut",this);
	...
	var insidePanel = bkClass.extend({
		construct : function(elm,nicEditor,options,openButton) {
			...
			var p=$(elm).parents(".nicEdit-buttonContain").parent();
...
//				}
		...
		addForm : function(f,elm) {
			...
			for(itm in f) {
				...
				if(type == 'title') {
						new bkElement('div').setContent(field.txt).setStyle({fontSize : '14px',color:"#000", fontWeight: 'bold', padding : '0px', margin : '2px 0'}).appendTo(this.form);
				} else {
					var contain = new bkElement('div').setStyle({overflow : 'hidden', clear : 'both'}).appendTo(this.form);
					if(field.txt) {
						new bkElement('label').setAttributes({'for' : itm}).setContent(field.txt).setStyle({margin : '2px 4px',color:"#000", fontSize : '13px', width: '50px', lineHeight : '20px', textAlign : 'right', 'float' : 'left'}).appendTo(contain);
					}
					...
					switch(type) {
						...
						case 'select':
							...
							for(opt in field.options) {
								new bkElement('option').setAttributes({value : opt, selected : (opt == val) ? 'selected' : ''}).setContent(field.options[opt]).appendTo(this.inputs[itm]);
							}
							break;
						case 'content':
...
//	nicEditors.registerPlugin(nicPlugin,nicLinkOptions);
	...
	var nicUploadButton = nicEditorAdvancedButton.extend({
		...
		addForm : function() {
			...
			myDoc.write('<div style="font-size: 14px;color:#000 font-weight: bold; padding-top: 5px;">插入图片</div>');
